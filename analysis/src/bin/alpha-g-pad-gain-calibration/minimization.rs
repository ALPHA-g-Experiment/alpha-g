...
pub(crate) fn try_minimization(
    ...
    // data suppression threshold in cathode pad waveforms is approximately
    // 100. The range of the waveform is approximately between [-2048, 2047].
    // Then a scaling factor of 10 corresponds to an equivalent suppression of
    // 1000 in another pad. This is already excessive, so any
    // value above 10 is not even worth dealing with (at that point it might
    // be better to mask that channel).
